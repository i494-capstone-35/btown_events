...
A={},v=0,j=0,s=0,z=0,F=0,E=false,J=[],R=false,T=a.support.scrollTop,N=a.support.eventCapture,K=a(d),H=1,C=0;a.vmouse={moveDistanceThreshold:10,clickDistanceThreshold:10,resetTimerDuration:1500};for(var I=0;I<y.length;I++)a.event.special[y[I]]=q(y[I]);N&&d.addEventListener("click",function(n){var u=J.length,t=n.target;if(u)for(var B=n.clientX,G=n.clientY,M=a.vmouse.clickDistanceThreshold,L=t;L;){for(var Q=0;Q<u;Q++){var O=J[Q];if(L===t&&Math.abs(O.x-B)<M&&Math.abs(O.y-G)<M||a(L).data(w)===O.touchID){n.preventDefault();
...
j.jqmData("title")||j.find(".ui-header .ui-title").text();if(P&&Q==document.title)Q=P;!M&&!L&&h.addNew(C,s,Q,j);document.title=h.getActive().title;d();a.mobile.silentScroll(j.jqmData("lastScroll"));P=j;var U=P.find(".ui-title:eq(0)");U.length?U.focus():P.find(k).eq(0).focus();H&&H.data("page")._trigger("hide",null,{nextPage:j});j.data("page")._trigger("show",null,{prevPage:H||a("")});a.mobile.activePage=j;B!==null&&B.remove();b.removeClass("ui-mobile-rendering");J()}function V(P){a.mobile.pageContainer.addClass(P);
W.push(P)}a.mobile.silentScroll();var S=e.scrollTop(),X=["flip"],W=[];if(C.indexOf("&"+a.mobile.subPageUrlKey)>-1)j=a(":jqmData(url='"+C+"')");if(H){H.jqmData("lastScroll",S);H.data("page")._trigger("beforehide",null,{nextPage:j})}j.data("page")._trigger("beforeshow",null,{prevPage:H||a("")});if(s&&s!=="none"){a.mobile.pageLoading(true);a.inArray(s,X)>=0&&V("ui-mobile-viewport-perspective");V("ui-mobile-viewport-transitioning");if(H)H.addClass(s+" out "+(z?"reverse":""));j.addClass(a.mobile.activePageClass+
" "+s+" in "+(z?"reverse":""));j.animationComplete(function(){j.add(H).removeClass("out in reverse "+s);H&&H.removeClass(a.mobile.activePageClass);O();a.mobile.pageContainer.removeClass(W.join(" "));W=[]})}else{a.mobile.pageLoading(true);H&&H.removeClass(a.mobile.activePageClass);j.addClass(a.mobile.activePageClass);O()}}function T(){if(m||j.jqmData("role")==="dialog"){C=h.getActive().url+p;if(m){j.attr("data-"+a.mobile.ns+"role",m);m=null}}j.page()}var N=a.type(j)==="array",K=a.type(j)==="object",
H=N?j[0]:a.mobile.activePage;j=N?j[1]:j;var C=a.type(j)==="string"?f.stripHash(j):"",I=C,n,u="get",t=false,B=null,G=h.getActive(),M=false,L=false,Q=document.title;if(!(G&&h.stack.length>1&&G.url===C&&!N&&!K))if(l)o.unshift(arguments);else{l=true;E&&h.directHashChange({currentUrl:C,isBack:function(){L=!(M=true);z=true;s=s||G.transition},isForward:function(){L=!(M=false);s=s||h.getActive().transition}});if(K&&j.url){C=j.url;n=j.data;u=j.type;t=true;if(n&&u==="get"){if(a.type(n)==="object")n=a.param(n);
C+="?"+n;n=c}}D&&D.reset();if(window.document.activeElement)a(window.document.activeElement||"").add("input:focus, textarea:focus, select:focus").blur();if(C){j=a(":jqmData(url='"+C+"')");I=f.getFilePath(C)}else{N=j.attr("data-"+a.mobile.ns+"url");K=f.getFilePath(N);if(N!==K)I=K}if(s===c)s=m&&m==="dialog"?"pop":a.mobile.defaultTransition;if(j.length&&!t){I&&D&&D.set(I);T();R()}else{if(j.length)B=j;a.mobile.pageLoading();a.ajax({url:I,type:u,data:n,dataType:"html",success:function(O){var V=a("<div></div>"),
S,X=O.match(/<title[^>]*>([^<]*)/)&&RegExp.$1,W=RegExp("\\bdata-"+a.mobile.ns+"url=[\"']?([^\"'>]*)[\"']?");if(RegExp(".*(<[^>]+\\bdata-"+a.mobile.ns+"role=[\"']?page[\"']?[^>]*>).*").test(O)&&RegExp.$1&&W.test(RegExp.$1)&&RegExp.$1)S=RegExp.$1;if(S){D&&D.set(S);C=I=f.getFilePath(S)}else D&&D.set(I);V.get(0).innerHTML=O;j=V.find(":jqmData(role='page'), :jqmData(role='dialog')").first();if(X)Q=X;if(!a.support.dynamicBaseTag){var P=f.get(I);j.find("[src], link[href], a[rel='external'], :jqmData(ajax='false'), a[target]").each(function(){var U=
a(this).is("[href]")?"href":"src",Y=a(this).attr(U);Y=Y.replace(location.protocol+"//"+location.host+location.pathname,"");/^(\w+:|#|\/)/.test(Y)||a(this).attr(U,P+Y)})}j.attr("data-"+a.mobile.ns+"url",I).appendTo(a.mobile.pageContainer);T();setTimeout(function(){R()},0)},error:function(){a.mobile.pageLoading(true);d(true);D&&D.set(f.get());J();a("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>"+a.mobile.pageLoadErrorMessage+"</h1></div>").css({display:"block",opacity:0.96,top:a(window).scrollTop()+
